...
export function MessageReactions({ children, reactions = {}, sessionId, onReact, actions, onLongPress, align = 'start', showMobileReaction = false }: {
  ...
  return <>
    <div className={`relative flex min-w-0 w-fit max-w-full flex-col ${align === 'end' ? 'self-end items-end' : 'self-start items-start'}`}>
      ...
      <div className="hidden w-max max-h-0 flex-nowrap items-center gap-0.5 overflow-hidden opacity-0 transition-[max-height,opacity] duration-150 group-hover:max-h-8 group-hover:opacity-100 group-focus-within:max-h-8 group-focus-within:opacity-100 sm:flex">
