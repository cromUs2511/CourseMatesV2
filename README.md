...
Messages are held in a bounded RAM buffer (500 messages per room), with a 4,000-character message limit. Leaving or signing out clears the room for both peers. Abandoned rooms expire after roughly 30–35 seconds without polling/heartbeat. Refresh can resume an existing room by starting the matching flow again while that room is still active. Restarting the server clears all sessions and rooms.
