...
- `GEMINI_API_KEY` powers the Student Chatbot Assistant and generated conversation starters. Existing deployments may use `Gemini_AI` as an alias. `GEMINI_MODEL` defaults to `gemini-3.6-flash` and can be changed to a model available to your project. Without a key, local starters remain available, while the chatbot, summaries, and explanations report that AI is unavailable instead of inventing results.
