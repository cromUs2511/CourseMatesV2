...
import { CHAT_IMAGE_TYPES, MAX_CHAT_IMAGES, MAX_GIF_BYTES, MAX_IMAGE_BYTES, MAX_IMAGE_DIMENSION, MAX_MESSAGE_MEDIA_BYTES } from './src/data/chatImages';
...
export function parseImages(value: unknown): StoredImage[] {
  ...
  const parsed = value.map(image => {
    if (!image || typeof image.dataUrl !== 'string' || image.dataUrl.length > Math.ceil(MAX_GIF_BYTES / 3) * 4 + 40) {
    ...
    if (!match || !CHAT_IMAGE_TYPES.includes(match[1])) throw new Error('Choose a JPEG, PNG, WebP, or GIF image.');
    ...
    const validHeader = mimeType === 'image/jpeg' ? bytes.subarray(0, 3).equals(Buffer.from([0xff, 0xd8, 0xff]))
      ...
      : mimeType === 'image/gif' ? ['GIF87a', 'GIF89a'].includes(bytes.toString('ascii', 0, 6))
      : bytes.toString('ascii', 0, 4) === 'RIFF' && bytes.toString('ascii', 8, 12) === 'WEBP';
    const byteLimit = mimeType === 'image/gif' ? MAX_GIF_BYTES : MAX_IMAGE_BYTES;
    if (!validHeader || !bytes.length || bytes.length > byteLimit || bytes.toString('base64') !== match[2]) {
  ...
  if (parsed.reduce((total, image) => total + image.bytes.length, 0) > MAX_MESSAGE_MEDIA_BYTES) throw new Error('Attachments must total 4 MB or less per message.');
  return parsed;
}
